...
- **Update on every read** - before returning the list, run `UPDATE Shipment SET isLate = true WHERE ("currentStatus" != 'DELIVERED' AND "promisedDeliveryDate" < NOW()) OR ("currentStatus" = 'DELIVERED' AND "deliveredAt" > "promisedDeliveryDate")`. This works, but now every `GET /api/shipments` call does a write as a side effect. A GET request mutating data is not standardised way of doing so.
...
- **`findCustomerById` instead of `findAllCustomers` in `createShipment`.** The service currently calls `findAllCustomers()` and iterates in memory to check if the customer exists. A dedicated `findCustomerById(id)` repository function would perform a single `SELECT WHERE id = ?` which directly jumps to the requested value without full table scan and that's it, instead of iterating through the list of customers which will require full table scan just to check if customer exists. At five seed customers the difference is invisible, but if we had like 100 000 customers, there would be performance issue.
